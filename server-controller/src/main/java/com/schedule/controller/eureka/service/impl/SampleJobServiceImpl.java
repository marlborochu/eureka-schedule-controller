 ...
 */
@ConditionalOnProperty(name = Constant.KEY_SCHEDULE_CONTROLLER_SAMPLE_MANAGER, matchIfMissing = false, havingValue = "true")
...
public class SampleJobServiceImpl implements JobService {
	...
	public void cancel() {
		...
		if (!taskInfos.isEmpty()) {
			logger.info("begin cancel all task....");
			synchronized (taskInfos) {
				Iterator<String> keys = taskInfos.keySet().iterator();
				while (keys.hasNext()) {
					String jobId = keys.next();
					HashMap<String, Object> info = taskInfos.get(jobId);
					((ScheduledFuture<?>) info.get(Constant.KEY_TASK_INFO)).cancel(true);
				}
				taskInfos.clear();
			}
		}
	...
	private void createTask(JobInfo job) {
		...
		try {
			...
			if (validateJobExecTime(job)) {
				...
				logger.error("can't create task [" + job.getJobId() + "] begin time is [" + job.getBeginDateTime()
						+ "] and end time is [" + job.getEndDateTime() + "]");
			}
		} catch (Exception e) {
